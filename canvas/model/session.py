...
class _Session:
	...
	def __init__(self):
		#	Eagerly create a connection and cursor since psycopg2 is lazy in 
		#	actualizing them.
		self.conn = connect(**config['database'])
		self.cursor = self.conn.cursor()
		
		#	Stores the active row reference to model instance mapping.
		self.active_mappings = {}

	def _row_reference(self, model):
		'''
		The canonical row reference format for the row reference to model 
		instance mapping.
		'''
	...
	def _map_model(self, model, row):
		'''
		Set values on a model object from a row in the corresponding table.
	...
	def _update_reference(self, model):
		...
		#	Store referenced primary key value on model and in internal 
		#	mapping.
	...
	def _precheck_constraints(self, model):
		'''
		Check for cumulative constraint violations on a model object 
		preceeding a commit, raising a `ValidationErrors` if any exist 
		and doing nothing otherwise.
		'''
		#	Check each constraint and collect those violated in a constraint 
		#	name to error message mapping.
		error_dict = {}
		for name, column in model.__class__.schema_iter():
			...
			if to_check == _sentinel:
				#	Skip unset values, this is a call to `save()` and defaults 
				#	aren't populated.
				continue
	...
	def _load_model(self, model_cls, row):
		'''
		Map a row to a model. If the row is already mapped by this session, 
		update the mapping and return the existing object. Otherwise, create 
		a new instance and map it.
		'''
		#	Create the row reference. An invariant within all database reads 
		#	and write is that primary keys are referenced first in SQL.
		...
		if remap:
			...
			#	Create the model object manually so `__init__` usage is intuative.
	...
	def save(self, model):
		...
		#	Check constraints before insertion so a cumulative exception can 
		#	be raised if multiple constraints are violated.
	...
	def query(self, target, conditions=True, one=False, order_by=None, descending=False):
		...
		:target The model class (must have been decorated with 
			`model.schema()`).
		:conditions A query condition.
		:one Whether to return the first result only, or `None` if there are 
			not results.
		'''
