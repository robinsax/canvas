...
from .utils import logger, format_traceback
from .core.plugins import get_path_occurrences
from . import CANVAS_HOME, application
...
class TestSuite:
	...
	def __init__(self, name):
		'''
		Create a decorator for registering a new test suite called `name`.
		'''
...
class Fail(Exception):
	'''
	Raised to fail a test. Any other exception being propogated will cause the 
	test to be considered broken.
	'''
	pass
...
def check_throw(trigger, ex_cls, message='Generic throw check'):
	'''
	Perform a check for a thrown exception, failing the build if it fails.
	'''
...
def check_json_response(response, status, json_valid, name):
	'''
	Perform a check on a JSON response produced by a testing client.
	'''
	try:
		check((
...
def check_html_response(response, status, html_valid, name):
	...
	check((
		response.status_code == status and
		html_valid(response.data.decode('utf-8'))
	), name)

def run_tests(suites):
	'''
	Run all registered tests, returning `True` if the build is passing and 
	`False` otherwise.
	'''
	#	Import the core suites.
		...
		for name, test_fn, fail_fn in suite.tests:
			...
			try:
				...
				if fail_fn is not None:
					#	Allow the test to clean up any resources it was using.
					fail_fn()
